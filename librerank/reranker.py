    ...
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, itm_spar_num, itm_dens_num,
        ...
        # ===== define placeholders ===== #
            ...
            with tf.name_scope('embedding'):
                ...
                self.itm_spar_emb = tf.gather(self.emb_mtx, self.itm_spar_ph)  # [b,l,d]
...
class PIER(BaseModel):
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, itm_spar_num, itm_dens_num,
        ...
        super(PIER, self).__init__(feature_size, eb_dim, hidden_size, max_time_len,
                                itm_spar_num, itm_dens_num, profile_num, max_norm)

        with self.graph.as_default():

            self.item_seq = self.multihead_attention(self.item_seq, self.item_seq, num_units=d_model, num_heads=n_head)
            self.item_seq = self.build_mlp_net(self.item_seq, layer=(200, 100, d_model))
    ...
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, itm_spar_num, itm_dens_num,
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("network"):
                ...
                new_dense_feature, _ = tf.nn.dynamic_rnn(GRUCell(hidden_size), inputs=layer1,
                                                        sequence_length=self.seq_length_ph, dtype=tf.float32,
                                                        scope='gru')
