...
def repeat_data(data, rep_num):  # [7,16,10], 5
    
    for i in range(len(data)):
        data_i = np.array(data[i]) # [16, 10, ?]
        shape = data_i.shape 
...
def get_expert_aggregated_batch(data):
    
    click_array = np.array(data[4])  # [b,l]
    action_index = np.empty(click_array.shape, dtype=int)

    for i in range(click_array.shape[0]):
        click_indices = np.where(click_array[i] == 1.0)[0]
        nclick_indices = np.where((click_array[i] == 0) | (click_array[i] == 0.0))[0]
        action_index[i] = np.concatenate((click_indices, nclick_indices))

    for i in range(2, len(data)-1):
        data[i] = np.array(data[i])
        data[i] = data[i][np.arange(data[i].shape[0])[:, None], action_index].tolist()
